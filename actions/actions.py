...
class Blenderbot(Action):
    ...
    def run(self, dispatcher: CollectingDispatcher,
        ...
        with app.app_context():
            ...
            # myobj = json.dumps({"userText": userInput})
            reply = requests.post("http://127.0.0.1:9090/bot", json = jsonify({"userText": userInput}))
            # reply = json.loads(reply)
            dispatcher.utter_message(text=str(reply.json))
            return []
